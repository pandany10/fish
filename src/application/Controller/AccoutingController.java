...
public class AccoutingController extends Menu implements Initializable {
	@FXML
	...
	public void isShowSearchCus(boolean isShow){
		//if(isShow == true){
			...
			lblstatuspdf.setVisible(!isShow);
		//}
	}
...
/*		keySearch.addEventHandler(KeyEvent.KEY_RELEASED, new EventHandler<KeyEvent>() {
	...
	public void Report11(ActionEvent event) throws IOException {
		Report11();
	}
...
;
	...
	public void Report11() throws IOException { 
		...
		if(check == false){
			...
			btnReport11.setDisable(true);
			btnReport11.setText("Generate Historical Report");
			...
			Thread thLoadData = new Thread() {
				...
				public void run() {
						...
						List<OrderModel> lstOrder;
						try {
							lstOrder = orderDao.getOrderCustomerSale1(customer.getCustomerID());
								pdf.Prints1(lstOrder,tddate);
								Platform.runLater(new Runnable() {
									  @Override
									  public void run() {
										  btnReport11.setDisable(false);
										    btnReport11.setText("Historical Report");
									  }
								});
							...
							e.printStackTrace();
							btnReport11.setDisable(false);
							btnReport11.setText("Aging Report");
						}
	;

...
import application.Dao.ExpressDao;
...
import application.Utill.PdfExpress;
...
public class OrdersExpressStoreController extends Menu implements Initializable {
	...
	final String[] filterSote = new String[] { "All", "By Store" };

	String screen = "final";
	
	public boolean stateEdit = false;
	
	public boolean stateStore = false;
	...
	public void getOrderExpressAll(){
		twOrder.getItems().clear();
		Thread thLoadData = new Thread() {
			@SuppressWarnings("deprecation")
			public void run() {
				try {
					list = orderDao.getOrder(cusID, "ExpressStore");
					twOrder.getItems().clear();
	...
	@FXML
	public ChoiceBox cbFilterStore;
	...
	public void setStatus(Boolean hiden) {
		...
		if(hiden == false){
			...
			twOrder.setLayoutY(1);
			twOrder.setPrefHeight(689);
		}
	}
;
	@FXML
	...
	public void actionSearch() throws IOException {
    	...
    	try {
    		...
    		List<ProductModel> lstProduct = productDao.getListProductSearchExpress(txtKeySearch.getText(),str_fSearchPs);
	...
	public void initViewOrders() {
		...
		cbFilterStore.setItems(FXCollections.observableArrayList("All", "By Store"));
		cbFilterStore.setValue("All");
		cbFilterStore.getSelectionModel().selectedIndexProperty()
		.addListener(new ChangeListener<Number>() {
			public void changed(ObservableValue ov, Number value, Number new_value) {
				String str_filters = filterSote[new_value.intValue()];
				if(str_filters.equals("All")){
					getOrderExpressAll();
					stateStore = false;
					isShowOrdes(true);
				}
				if(str_filters.equals("By Store")){
					//
					stateStore = true;
					isShowOrdes(true);
					try {
	...
	public void EditOrder() throws ClassNotFoundException, SQLException {
		...
		editOrderId = in;
		isShowOrdes(false);
		orderDetail = orderDao.getOrderDetailExpress(editOrderId);
		showDetail(orderDetail);
	}
/*	public void showDetail(int orderId) throws ClassNotFoundException, SQLException {
	...
	public void gotoPrintInvoice() throws IOException {
		...
		try {
			expressModel = expressDao.getSite(customer.getCustomerID());
			
		} catch (ClassNotFoundException | SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		String chlSL = orderDetail.getLstProduct().get(0).getChlSL();
		pdf.PrintInvoiceExpress(orderDetail ,currentOrders,expressModel,chlSL);
		try {
	...
	public void isShowOrdes(Boolean isShowOrdes) {
		...
		if (!init) {
			if (isShowOrdes) {
				prevStage.setTitle("Orders Express");
			} else {
				prevStage.setTitle("Edit Order Express id " + editOrderId);
			}
		} else {
			init = false;
		}
		if(stateStore == true && stateEdit == false){
			orders.setVisible(isShowOrdes);
			store.setVisible(isShowOrdes);
			prevStage.setTitle("Orders Express");
		}else{
	...
	public void getStoreExpress() throws IOException {
	  	  CustomerDao customerDao = new CustomerDao();
	  	  twSearchCus.getItems().clear();
	  	  twSearchCus.setPlaceholder(new Label("Please wait… Searching Database."));
	  	  try {
	  		long start = System.nanoTime();    
	  		List<CustomerModel> lstCustomer = customerDao.getLstCustomerForExpress();
	  		long elapsedTime = System.nanoTime() - start;
	  		double seconds = (double)elapsedTime / 1000000000.0;
	  		System.out.println("selconds: "+seconds);
	  		// show result
	  		...
	  		if(lstCustomer.size() == 0){
	  			twSearchCus.setPlaceholder(new Label("No found."));
	  		}
			} catch (ClassNotFoundException | SQLException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
